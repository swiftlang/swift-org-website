...
Download swiftly for [Linux (Intel)](https://download.swift.org/swiftly/linux/swiftly-#(swiftlyVersion)-x86_64.tar.gz), or [Linux (ARM)](https://download.swift.org/swiftly/linux/swiftly-#(swiftlyVersion)-aarch64.tar.gz).

```
curl -O "https://download.swift.org/swiftly/linux/swiftly-#(swiftlyVersion)-$(uname -m).tar.gz"
```
...
curl -O "https://download.swift.org/swiftly/linux/swiftly-#(swiftlyVersion)-$(uname -m).tar.gz.sig"
gpg --verify swiftly-#(swiftlyVersion)-$(uname -m).tar.gz.sig swiftly-#(swiftlyVersion)-$(uname -m).tar.gz
```

Extract the archive.

```
tar -zxf swiftly-#(swiftlyVersion)-$(uname -m).tar.gz
```
...
swift --version
--
Swift version #(latestRelease) (swift-#(latestRelease)-RELEASE)
