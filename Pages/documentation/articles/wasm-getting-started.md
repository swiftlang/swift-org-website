...
2. Install Swift #(releaseName) with `swiftly install #(releaseName)`.

3. Select the installed toolchain with `swiftly use #(releaseName)`.
...
```
swift build --swift-sdk #(releaseTag)_wasm
```
...
```
swift run --swift-sdk #(releaseTag)_wasm
```
...
```
swift build --swift-sdk #(releaseTag)_wasm-embedded
```

or

```
swift run --swift-sdk #(releaseTag)_wasm-embedded
```
...
5. Create a `.sourcekit-lsp/config.json` file in your project root:
   ...
   {
       "swiftPM": {
           "swiftSDK": "#(releaseTag)_wasm"
       }
   }
   ```

   Replace `#(releaseTag)_wasm` with the exact Swift SDK ID from your `swift sdk list` output. Use `#(releaseTag)_wasm-embedded` if you're working with Embedded Swift.
...
2. Create a `.sourcekit-lsp/config.json` file in your project root:
   ...
   Replace `#(releaseTag)_wasm` with your Swift SDK ID from `swift sdk list`. Use `#(releaseTag)_wasm-embedded` for Embedded Swift.
