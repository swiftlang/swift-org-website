...
The Swift 5.7 language and standard library:
  * Adds new shorthand syntax for common boilerplate code, including `if let` statements and multi-statement closure type annotations,
  * Lifts long-standing language limitations to make generic programming more seamless,
  * Enhances data race safety with new annotations and opt-in diagnositcs,
  * Brings actor isolation to distributed environments,
  * Improves the usability of a suite of existing pointer APIs, and
  * Provides brand new language support and APIs for string processing.
...
The following language and standard library proposals were accepted through the [Swift Evolution](https://github.com/apple/swift-evolution) process and [implemented in Swift 5.7](https://apple.github.io/swift-evolution/#?version=5.7).
