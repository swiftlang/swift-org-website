...
These new features are especially useful on Linux, which, unlike
Apple platforms and Windows, often doesn't have a system-wide crash
reporter so very likely all you get when your program fails is a
message from the parent process (often the shell) telling you that the
...
result is now something like this:
...
non-Apple platforms the backtracer needs to be able to look up symbols
